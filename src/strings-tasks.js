...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1)
    if (vowels.includes(string[i])) countVowel += 1;
...
function isPalindrome(str) {
  const removeChar = str.replace(/\W/g, '').toLowerCase();
...
function extractNameFromTemplate(value) {
  const removeChar = value.replace(/\W/g, ' ');

  return removeChar.slice(6).trim();
}
...
function encodeToRot13(str) {
  return str.replace(/[a-z]/gi, (s) =>
    String.fromCharCode(s.charCodeAt(0) + (s.toLowerCase() <= 'm' ? 13 : -13))
  );
}
